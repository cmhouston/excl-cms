...
namespace WP_API_Custom\UseCases
{
    ...
    class PartnerCommenter
    {
        ...
        {
            log2('inside getPartnerComments');
            $comments = $this->wordpress->get_comments('post_id='.$postId);
        ...
        protected function prepare_author( $author ) {
            $user = $this->wordpress->get_user_by( 'id', $author );
            $partner = get_user_meta($user->ID, 'partner');
        ...
        protected function prepare_comment( $comment, $requested_fields = array( 'comment', 'meta' ), $context = 'single' ) {
            ...
            // Author
            if ( (int) $comment->user_id !== 0 || $comment->user_id !== null) {
            ...
            // Date
            $fields['date'] = date( 'c', strtotime( $comment->comment_date ) );
